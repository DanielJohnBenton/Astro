...
function MaybeGem(Mode:Number, atX:Number, atY:Number):void
{
	...
	*/
				...
				(RandomNumber(1, Frustration) == 1))
	{
		...
		if(RandomNumber(1, 4) == 2)
		{
			//GemMode = 1; // health refill
		}
		else if(RandomNumber(1, 4) == 2)
		{
			GemMode = 3; // better bullet
		}
		else if(RandomNumber(1, 4) == 1)
		{
			GemMode = 2; // +1 life / uberbullet
		}
		else if(RandomNumber(1, 4) == 3)
		{
			GemMode = 4; // backwards firing
		}
		else if(RandomNumber(1, 5) == 2)
		{
			GemMode = 5; // 3 bullets
		}
		else if(RandomNumber(1, 4) == 2)
		{
			GemMode = 6; // circling bullet
		}
		else if(RandomNumber(1, 5) == 3)
		{
			GemMode = 7;
		}*/
		...
		var gs = RandomNumber(1, 100);
		
		if(IsBetween(gs, 1, 30))
		{
			GemMode = 1; // health refull
		}
		else if(IsBetween(gs, 30, 50))
		{
			GemMode = 3; // better bullet
		}
		else if(IsBetween(gs, 50, 70))
		{
			GemMode = 2; // life++ or uberbullet
		}
		else
		{
			gs = RandomNumber(1, 100);
			if(IsBetween(gs, 1, 30))
			{
				GemMode = 4; // firing backwards
			}
			else if(IsBetween(gs, 30, 60))
			{
				GemMode = 5; // 3 bullets
			}
			else
			{
				gs = RandomNumber(1, 100);
				if(IsBetween(gs, 1, 70))
				{
					GemMode = 6; //circling bullet
				}
				else
				{
					GemMode = 7; // homing bullet
				}
			}
		}*/
		
		/*var gs = RandomNumber(1, 10);
		
		if(IsBetween(gs, 1, 3))
		{
			GemMode = 1;
		}
		else if(IsBetween(gs, 4, 6))
		{
			GemMode = 3;
		}
		else
		{
			var GemMode = RandomNumber(1, 7);
		}*/
...
function GameLoop(event:Event):void
{
	...
	//TimeSinceLastFrame = NewTime - LastTime;
	//LastTime = NewTime;
